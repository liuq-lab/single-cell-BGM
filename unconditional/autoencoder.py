...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--selection-n-eval", type=int, default=2000)
...
def validation_sample_ilisi(
    ...
    """One fixed-seed NB reconstruction iLISI for AE checkpoint selection.

    The same validation cells and the same NB random seed are used at every
    evaluation epoch.  The score is computed after decoding back to full-gene
    expression, normalize_total(1e4)+log1p, joint PCA20, and KNN10 iLISI.
    """
    ...
    raw = dense_batch(validation.X, indices)
    real = normalize_total_log1p_dense(raw, target_sum=target_sum)
    latent = encode_matrix(model, raw, batch_size=batch_size, device=device)
    library_size = raw.sum(axis=1).astype(np.float32)
    reconstructed = decode_to_log_expression(
        model,
        latent,
        library_size,
        mode="sample",
    ...
    n = len(real)
    joint = np.concatenate([real, reconstructed], axis=0)
    n_components = min(20, joint.shape[0] - 1, joint.shape[1])
    pcs = PCA(
    ...
    graph = ad_local.AnnData(X=np.zeros((2 * n, 1), dtype=np.float32))
    graph.obsm["X_metric_pca20"] = pcs.astype(np.float32)
    graph.obs["batch"] = pd.Categorical(["real"] * n + ["reconstructed"] * n)
    sc.pp.neighbors(
        graph,
        use_rep="X_metric_pca20",
        n_neighbors=10,
        random_state=seed,
    )
    ilisi = float(scib.me.ilisi_graph(graph, batch_key="batch", type_="knn"))
    reconstructed_zero = float(np.mean(reconstructed <= 0))
    return ilisi, reconstructed_zero
...
def main() -> None:
    ...
    best_ilisi = -float("inf")
    best_epoch = -1
    best_epoch_val_nll = float("nan")
    best_path = output / "model.pt"
    rng = np.random.default_rng(args.seed + 100)

    selection_n = min(int(args.selection_n_eval), int(validation.n_obs))
    selection_rng = np.random.default_rng(args.selection_seed)
    selection_indices = selection_rng.choice(
        validation.n_obs,
        size=selection_n,
        replace=False,
    ).astype(np.int64)
    np.save(output / "selection_validation_indices.npy", selection_indices)
    print(
        "AE checkpoint selection: max single-sample validation reconstruction "
        f"PCA20 iLISI on {selection_n} fixed validation cells"
    )
    print("Selection NB/PCA seed:", args.selection_seed)
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        if should_eval:
            ...
            val_ilisi, val_recon_zero = validation_sample_ilisi(
                model,
                validation,
                selection_indices,
            ...
            row = {
                ...
                "validation_reconstruction_ilisi_pca20": val_ilisi,
                "validation_reconstruction_zero_frac": val_recon_zero,
            ...
            print(
                ...
                f"val_recon_iLISI={val_ilisi:.6f} "
                f"val_recon_zero={val_recon_zero:.6f}"
            )
            if np.isfinite(val_ilisi) and val_ilisi > best_ilisi:
                best_ilisi = val_ilisi
                best_epoch = epoch
                best_epoch_val_nll = val_nll
                torch.save(model.state_dict(), best_path)
                print(
                    f"  new best AE checkpoint: epoch={epoch} "
                    f"validation reconstruction iLISI={val_ilisi:.6f}"
                )
    ...
    metadata = {
        ...
        "checkpoint_selection": "max_single_sample_validation_reconstruction_ilisi_pca20",
        "best_validation_reconstruction_ilisi_pca20": float(best_ilisi),
        "selected_epoch_validation_nll_per_cell": float(best_epoch_val_nll),
        "selected_epoch_validation_nll_per_gene": float(best_epoch_val_nll / train.n_vars),
        "selection_n_eval": int(selection_n),
        "selection_seed": int(args.selection_seed),
        "selection_nb_replicates": 1,
        "selection_pca_components": 20,
        "selection_neighbors": 10,
    ...
    print("Validation reconstruction iLISI:", best_ilisi)
    print("Selected-epoch validation NLL/gene:", best_epoch_val_nll / train.n_vars)
    gc.collect()
